...
inline constexpr SignedWord kImmediateMin = -(SignedWord{1} << (kPayloadBits - 1));
inline constexpr SignedWord kImmediateMax = (SignedWord{1} << (kPayloadBits - 1)) - 1;
...
[[nodiscard]] constexpr Operand Imm(SignedWord v) noexcept { return MakeTagged(WordTag::kImmediate, static_cast<Word>(v) & kPayloadMask); }
...
[[nodiscard]] constexpr bool IsImmediate(Operand w) noexcept { return IsTag(w, WordTag::kImmediate); }
