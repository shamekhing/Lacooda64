...
// Encodes a Trace (vector of Instructions) into a flat Word stream (4 Words each).
...
enum class DecodeError : Word {
    None = 0,
    MisalignedWordCount,   // The word count was not a multiple of 4.
    InvalidInstruction,    // A decoded instruction failed validation.
};
...
struct DecodeResult {
    ...
    Word wordOffset{0};     // Word index where the error occurred.
...
[[nodiscard]] inline DecodeResult decode(const ProgramWords& words) noexcept {
    DecodeResult out{};
    if ((words.size() % 4) != 0) {
        out.error = DecodeError::MisalignedWordCount;
        out.wordOffset = static_cast<Word>(words.size());
        return out;
    }

    out.trace.reserve(words.size() / 4);
    for (std::size_t i = 0; i < words.size(); i += 4) {
        Instruction inst{words[i], words[i+1], words[i+2], words[i+3]};
        if (!validInstruction(inst)) {
            out.error = DecodeError::InvalidInstruction;
            out.wordOffset = static_cast<Word>(i);
            return out;
        }
        out.trace.push_back(inst);
    }
    return out;
}
...
// Encodes a Trace into a raw little-endian byte buffer (32 bytes per instruction).
